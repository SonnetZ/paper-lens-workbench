    ...
    <aside
      ...
      className={
        collapsed
          ...
          : "paper-queue-index min-h-0 bg-swiss-wash lg:h-[100dvh] lg:overflow-hidden lg:border-r lg:border-swiss-rule"
      }
    >
      {collapsed ? (
        <button
      ...
      {collapsed ? null : (
        <>
          <div className="paper-queue-toolbar flex items-center justify-between gap-3 border-b border-swiss-rule bg-white px-4 py-2">
          ...
          <div className="paper-queue-search border-b border-swiss-rule px-4 py-3">
          ...
          <div className="paper-queue-list max-h-[calc(100dvh-370px)] overflow-auto">
            {filteredPapers.map((paper) => (
              <button
                key={paper.recordId}
                type="button"
                onClick={() => onSelect(paper.recordId)}
                className={`paper-queue-item block w-full border-b border-swiss-rule px-4 py-3 text-left transition active:translate-y-px ${
                  selectedRecordId === paper.recordId
                    ? "paper-queue-item-selected bg-white"
                    : "hover:bg-white"
                }`}
